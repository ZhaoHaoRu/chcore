...
int ipc_send_cap(struct ipc_connection *conn)
{
        ...
        for (i = 0; i < cap_slot_number; i++) {
                ...
                if (dest_cap < 0) {
                        kwarn("ipc_send_cap: something wrong when copy the cap to server\n");
                        goto out_free_cap;
                }
                cap_buf[i] = dest_cap;
                /* LAB 4 TODO END */
...
u32 sys_register_client(u32 server_cap, u64 vm_config_ptr)
{
        ...
        if (!server) {
                kinfo("sys_register_client: fail to get server\n");
                r = -ECAPBILITY;
                goto out_fail;
        }
...
u64 sys_ipc_call(u32 conn_cap, struct ipc_msg *ipc_msg, u64 cap_num)
{
        ...
        // IPC-msg is based on shared memory, calculate the correct offset.
